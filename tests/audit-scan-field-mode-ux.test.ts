...
test("audit scan compacts the sticky progress header after an asset is selected", () => {
  ...
  assert.match(form, /const queueTotal = hasAuditContext \? contextProgress\.total : items\.length/)
  assert.match(form, /const queueProcessedCount = hasAuditContext \? contextProgress\.processed : processedCount/)
  assert.match(form, /queueProcessedCount\.toLocaleString\("th-TH"\)/)
  assert.match(form, /queueTotal\.toLocaleString\("th-TH"\)/)
  assert.match(form, /queuePendingCount\.toLocaleString\("th-TH"\)/)
