...
- Import/Export Excel
- ตัวกรองด่วนสำหรับงานประจำ เช่น ข้อมูลไม่ครบ, ใกล้หมดประกัน, พร้อมใช้งาน, ใช้งานอยู่, ถูกเบิก, รอซ่อม, อยู่ระหว่างซ่อม และชุดคอลัมน์สำหรับมุมมองปฏิบัติการ/บัญชี/ตรวจนับ
...
- รองรับรูปทรัพย์สิน, รูปประจำรุ่น, เอกสารแนบ, PO/Invoice กลาง และข้อมูลเฉพาะตามประเภท โดยหน้าทะเบียนใช้รูปประจำรุ่นเป็น thumbnail หลักเพื่อให้รายการรุ่นเดียวกันดูสม่ำเสมอ ส่วนหน้ารายละเอียดแสดงรูปทะเบียนแบบย่อก่อนและมีปุ่มเปิดศูนย์หลักฐานเมื่อต้องค้นไฟล์จากทุก workflow
...
- หน้าพิมพ์ Label สามารถจัดคิวตามบริษัท สาขา หมวดหมู่ ที่ตั้ง และวันที่สร้างได้ โดยเพิ่มทั้งขอบเขตบริษัทหรือสาขาได้ก่อน แล้วค่อยเลือกที่ตั้งเฉพาะเมื่อจำเป็น
- ช่องค้นหาในหน้าพิมพ์ Label ใช้สำหรับค้นหาเฉพาะรายการเพิ่มเติม โดยพิมพ์อย่างน้อย 2 ตัวอักษร ส่วนงานพิมพ์เป็นล็อตให้เริ่มจากตัวกรองคิว
...
- หน้าสแกนบนมือถือแยกผลสแกนปัจจุบันออกจากรายการสแกนล่าสุดแบบกระชับ โดยแผงสแกนล่าสุดพับรายการไว้และแสดงจำนวนแบบ `แสดง X รายการล่าสุด` ไม่ใช่ยอดตรวจนับรวม พร้อมปุ่มหลักด้านล่างจอ เพื่อช่วยเดินตรวจจำนวนมากได้เร็วขึ้น
- โหมดเดินตรวจเร็วและสแกนต่อเนื่องเป็นค่าเริ่มต้นของหน้าสแกนตรวจนับ โดยไม่ต้องเปิด switch เพิ่มระหว่างเดินตรวจ
- ปุ่ม `คิวรอตรวจ` ในหัวหน้าสแกนช่วยเปิดรายการรอตรวจจากรอบเดิมได้ทันที โดยเป็นรายการแบบพับ/เปิดได้ แสดงที่ตั้ง/ผู้ถือครอง/แผนกที่คาดหวังบนการ์ด ไม่สร้างรอบตรวจใหม่ และถ้าเปิดรายการรอตรวจทั้งหมดจากหน้าสแกน จะเป็นหน้าการ์ดที่เหมาะกับมือถือ ค้นหาได้ และมีปุ่มสแกนรายการนี้/ดูทรัพย์สิน/บันทึกไม่พบ พร้อมปุ่มกลับที่พากลับมาสแกนต่อ โดยการบันทึกไม่พบใช้ dialog ในระบบ ไม่บังคับหมายเหตุ และแนบรูปหลักฐานได้ถ้ามี; ส่วนควบที่สแกนไม่ได้ก็ใช้ dialog รูปแบบเดียวกันจากแผงส่วนควบ
...
- อุปกรณ์ที่รองรับสามารถเปิดไฟฉายและกดซูม 1x/2x/3x ในหน้ากล้องสแกนได้เมื่อทำงานในจุดแสงน้อยหรือ QR Label มีขนาดเล็ก โดยกด 1x เพื่อกลับเป็นไม่ซูมได้โดยไม่ต้องปิดกล้อง
...
- เมนู รายการไม่ตรง เป็น workflow สำหรับจัดการ Finding เดิม โดยแยกงานรอ Review, งานที่ต้องแก้ไข, งานเกินกำหนด และงานที่ปิดแล้ว พร้อมเทียบข้อมูลในระบบกับข้อมูลที่พบจริงในหน้าเดียว มีเวลาโหลดข้อมูลล่าสุดแบบกะทัดรัด ปุ่มรีเฟรช เปิดไฟล์หลักฐานที่แนบกับ Finding ได้จากรายการ และเตือนเฉพาะเมื่อหน้าเปิดค้างจนข้อมูลอาจเก่า; ถ้าข้อมูลหลักของทรัพย์สินถูกแก้หลังเกิด Finding ระบบจะเตือนก่อนอนุมัติแก้ข้อมูลหลัก
