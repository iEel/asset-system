...
- The sticky progress header also has a `คิวรอตรวจ {count}` quick action. It opens an inline preview of pending `audit_items` for the current round so auditors can quickly load expected data, while the full pending list remains the place to mark an expected asset as not found.
- For high-volume mobile counts, auditors can switch into fullscreen walking mode. This keeps the same audit round and same scan/save logic, but makes the scanner, sticky progress, pending queue, and post-scan decisions easier to operate with one hand.
- In fullscreen walking mode, the current scan result appears as a bottom sheet with `บันทึกพบตรง`, `บันทึกข้อมูลไม่ตรง / Finding`, `ถ่ายรูปหลักฐาน`, and `สแกนต่อ / กรอกเอง`. The normal inline result panel remains for desktop and non-fullscreen use.
- In fullscreen walking mode, the pending queue is grouped by expected location so the field team can work zone by zone without creating a new audit round.
...
- Scan result feedback can use lightweight browser vibration on supported mobile devices. This is only a local UX cue and does not change audit history, findings, or offline queue behavior.
