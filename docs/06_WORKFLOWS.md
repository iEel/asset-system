...
- Scan printed Asset Label QR codes or manually enter asset identifiers. The Audit Scan camera uses the same native-resolution Asset QR decoder as `/asset-management/scan`, defaults to the rear-camera walking flow, and keeps continuous scan behavior for counting multiple labels in one session. QR scans stay exact/raw-value based for traceability; partial matching is manual-only.
...
- The scan result card is focused on the current QR/manual outcome. A separate compact `สแกนล่าสุด` panel keeps the newest three reads visible and lets auditors expand older reads, including matched, mismatch, out-of-scope, unknown asset, found-later, saved, and offline-queued results. Use this as a field sanity check; the official audit state still comes from the saved audit items, scan history, findings, and offline queue sync.
- Fast walking behavior and continuous scanning are the default Audit Scan behavior, so the normal field UI does not expose extra mode switches. Auditors choose `บันทึกพบตรง` to save a matched result immediately, or `บันทึกข้อมูลไม่ตรง / Finding` to open the detailed actual-value fields. On mobile, these post-scan actions move into a fixed bottom action bar together with `ถ่ายรูปหลักฐาน` and `สแกนต่อ / กรอกเอง` so field auditors can keep one hand on the phone.
