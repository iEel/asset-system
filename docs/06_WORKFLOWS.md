...
- If the camera cannot read the label or the auditor needs to pick an item manually, the old full asset dropdown is replaced by a collapsible searchable picker. With no search text it lists pending items first; once the user searches, it can find round items by Asset Tag, name, expected location, custodian, or department.
...
- In fast walking mode, auditors choose `บันทึกพบตรง` to save a matched result immediately, or `บันทึกข้อมูลไม่ตรง / Finding` to open the detailed actual-value fields. Fast Scan Mode and Continuous Scan now share a compact switch bar below the scan input, with short mobile labels and longer help kept for desktop/tooltip instead of a large instruction block. On mobile, these post-scan actions move into a fixed bottom action bar together with `ถ่ายรูปหลักฐาน` and `สแกนต่อ / กรอกเอง` so field auditors can keep one hand on the phone.
