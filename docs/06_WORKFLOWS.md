...
- A recent-scan list keeps the latest eight QR/manual scan outcomes in the current page session, including matched, mismatch, out-of-scope, unknown asset, found-later, saved, and offline-queued results. Use this as a field sanity check; the official audit state still comes from the saved audit items, scan history, findings, and offline queue sync.
- In fast walking mode, auditors choose `บันทึกพบตรง` to save a matched result immediately, or `บันทึกข้อมูลไม่ตรง / Finding` to open the detailed actual-value fields. On mobile, these post-scan actions move into a fixed bottom action bar together with `ถ่ายรูปหลักฐาน` and `สแกนต่อ / กรอกเอง` so field auditors can keep one hand on the phone.
