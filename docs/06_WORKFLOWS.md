...
- Use the Asset Register quick filters for common operational queues such as missing Serial, missing photo, purchase-info gaps, warranty expiring, missing responsibility, Ready, Pending Repair, and Under Maintenance. Quick filters are grouped by data quality, ownership/custody scope, and lifecycle status, update the URL, and preserve the current search/company/branch/category context. On mobile the groups scroll horizontally so the first asset cards are not pushed far below the fold.
