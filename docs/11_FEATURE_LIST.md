...
| Audit scan camera UX | Defaults to rear-camera walking mode, hides normal camera-ready status text and camera selector controls, shows camera utilities only while scanning, after a decoded QR reference exists, or when a camera issue needs attention; supported camera tracks expose a flashlight toggle for low-light scans and 2x/3x zoom buttons for small QR labels |
