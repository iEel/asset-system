...
| Audit scan | QR/manual scan, scan-first entry focus, compact Fast Scan Mode toggle, system data comparison panel, sticky single-source progress header with photo-queue badge and pending queue shortcut, combined scan-result/recent-scan panel, fullscreen mobile walking mode, fast `บันทึกพบตรง` save path, detailed `บันทึกข้อมูลไม่ตรง / Finding` path, photo evidence, and mobile bottom result actions after scan selection |
...
| Audit scan walking UX | Fullscreen walking mode keeps the same audit round while adding a one-hand result bottom sheet, expected-location queue grouping, and browser vibration cues where supported |
