...
> **Latest Update:** ✅ Settings now includes a structured depreciation policy builder for useful life, residual value, category groups, and calculation preview while keeping the existing `accounting_depreciation_policy` JSON setting. Brand / Model master data now uses a compact brand navigator plus model workspace layout, and System Settings Asset Tag Prefix UI uses prefix-group management while preserving the existing `categoryId -> prefix` JSON setting.
