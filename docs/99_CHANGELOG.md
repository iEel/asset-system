...
> **Latest Update:** ✅ System Settings Asset Tag Prefix UI now uses prefix-group management: operators enter a Prefix, move categories from available to selected lists with search, and save back to the existing `categoryId -> prefix` JSON setting. Storage Governance also gained message regression coverage so every `storagePage` key used by the page/archive button exists in Thai and English.
