...
| Integration API DB Token Manager | Replaced active token lifecycle configuration with DB-backed clients in `integration_api_clients`, managed through `Admin > Integration API` and `/api/admin/integration-clients` for create, rotate, disable, and enable. Plain tokens are returned once on create/rotate; the app stores only `tokenHash` and `tokenPreview`. Production must apply `prisma/manual-migrations/2026-06-14-add-integration-api-clients.sql` after backup/approval before deploy/use, while existing database connection settings remain unchanged. `npm run integration:token` remains recovery/troubleshooting tooling only. Emergency disable uses the admin UI or controlled SQL against `integration_api_clients.enabled`; code rollback is required before removing the table. |
