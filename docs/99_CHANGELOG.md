...
> **Latest Update:** ✅ Audit Rounds list is now action-first for high-volume audit operations, with next-action cards for continuing scans, reviewing mismatches, and closing ready rounds, URL-backed quick filters that preserve dashboard scroll position, and ready-to-close/blocker badges. Audit Scan still shows system data before the field decision, uses clearer `ข้อมูลตรง` / `ข้อมูลไม่ตรง` actions, keeps matched fast-walking saves photo-optional, requires at least one evidence photo when mismatched actual fields are saved, and supports multiple free-form evidence photos with optional grouping tags.
