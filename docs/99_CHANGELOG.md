...
| Area | Update |
|---|---|
| Cross-scope asset visibility | Asset Register quick filters now expose assets whose custodian company, custodian branch, or current location branch differs from the asset owner/tag scope. Reports adds a cross-scope summary and preview with drilldowns back to Asset Register, Asset Register export adds owner/custodian/home/current location branch columns and human-readable flags, and Asset Overview export adds cross-scope metrics plus a Cross Scope sheet. |
