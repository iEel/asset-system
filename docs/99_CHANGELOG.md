...
> **Latest Update:** ✅ Asset Label QR scanning on `/asset-management/scan` and Audit Scan now default to the generic rear camera selector on multi-camera devices, use an undistorted 4:3 mobile camera preview with a custom square QR guidance overlay, request focus-friendly 1280x960/30fps camera constraints with best-effort zoom/focus tuning, and decode directly from the native-resolution video frame through ZXing instead of the `html5-qrcode` CSS-pixel hidden canvas. The Asset QR path avoids mobile native `BarcodeDetector`, mirror-flip retries, and CSS video stretching so QR modules are not warped while users adjust distance until the QR itself is sharp; Audit Scan keeps continuous scan mode and stays in the audit workflow. Serial Number scanner inputs now use native-resolution ZXing `BrowserMultiFormatReader`, optional browser `BarcodeDetector`, focused scan-band canvas crops before full-frame fallback, QR plus common 1D/2D barcode formats, 1920x1080 camera constraints without forced serial digital zoom, and a narrower 16:9 scan-line guide for sharper manufacturer barcode capture. Settings still includes the structured depreciation policy builder, Brand / Model uses the compact brand navigator plus model workspace layout, and Asset Tag Prefix settings use prefix-group management while preserving the existing `categoryId -> prefix` JSON setting.
