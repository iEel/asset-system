...
| Adaptive Asset Register | Preserved the existing URL-backed filters, sorting, pagination, column presets, bulk selection/update, exports, import placement, permissions, and `returnTo` navigation while making responsive responsibilities explicit. Mobile cards now prioritize Asset Tag plus semantic status, name, serial/category metadata, location, custodian, and only field-relevant condition/shared-ownership warnings before detail/edit actions; company/branch and purchase price are omitted from the default card, while clone/delete actions are secondary. The desktop remains the dense operational table. Focused tests and ESLint passed; controller browser QA is pending. |
