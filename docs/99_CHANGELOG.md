...
> **Latest Update:** ✅ Employee My Assets landing added: linked employee self-service users now land on `/{locale}/my-assets` after login, direct `/dashboard` hits redirect there before global dashboard metrics are queried, and users with broader overview permissions still keep the normal dashboard.
