...
> **Latest Update:** ✅ Audit Scan now shows system data before the field decision, uses clearer `ข้อมูลตรง` / `ข้อมูลไม่ตรง` actions, keeps matched fast-walking saves photo-optional, requires at least one evidence photo when mismatched actual fields are saved, and supports multiple free-form evidence photos with optional grouping tags. Category create remains guarded from sending nested `deleteMany` into `assetCategory.create()`, and mobile QR/barcode scanning continues to use native-resolution ZXing decode paths.
