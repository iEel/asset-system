...
> **Latest Update:** ✅ Master-data count drilldowns now open the relevant Brand/Model workspace or Asset Register filter for category, brand, and model counts. Asset Register preserves Brand/Model drilldown filters, shows removable active filter chips, and uses a balanced five-column desktop filter grid with compact status/condition help icons so the right side no longer has an empty column.
