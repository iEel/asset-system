...
| Audit Scan field device UX | `/th/audit/rounds/{id}/scan` now shows a compact scan-result card plus the latest eight QR/manual scan outcomes for the current page session, with clear matched, mismatch, out-of-scope, unknown, found-later, saved, and offline-queued labels. Supported mobile camera tracks expose a flashlight toggle inside the scanner preview; unsupported browsers/devices hide the control and torch is turned off on scanner stop/camera switch. Audit workflow, scan history, findings, offline queue, permissions, and SOD behavior are unchanged. |
