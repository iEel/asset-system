...
> **Latest Update:** ✅ Brand / Model master data now uses a brand navigator plus model workspace layout, with active-only model/asset counts so soft-deleted models do not inflate the sidebar. System Settings Asset Tag Prefix UI now uses prefix-group management and filters stale/inactive category ids out of visible group counts while preserving the existing `categoryId -> prefix` JSON setting.
