...
> **Latest Update:** ✅ Asset lifecycle handoff now has a saveable status-flow diagram plus clearer status-vs-condition guidance. Asset create/edit blocks direct protected lifecycle status changes before submit, operators are pointed to the proper workflow, and maintenance ticket creation no longer requires `returnDate`; return date remains part of the close-ticket workflow.
