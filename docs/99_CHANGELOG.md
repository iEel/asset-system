...
> **Latest Update:** ✅ Permission-aware dashboard UX added: sidebar menus are filtered by the signed-in user's permissions, direct unauthorized page hits show `/{locale}/access-denied` instead of a generic 404, Topbar identity comes from the current session user, and System Settings > AD/LDAP Login uses a searchable active-role selector for the LDAP auto-provision default role.
