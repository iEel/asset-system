...
| Area | Update |
|---|---|
| Supplier management hardening | Corrected supplier lifecycle behavior so ordinary edits remain possible with linked records while deactivation and deletion are blocked by active assets, maintenance tickets, PM plans, or purchase documents. Replaced the mobile desktop-table scroller with supplier cards, bounded summary reads to count queries, added semantic sorting and actionable empty states, aligned form limits with the database, normalized optional blanks, mapped duplicate/validation failures to localized inline feedback, and added dirty-form protection. Supplier detail now uses a compact mobile metric strip and labelled purchase-document cards below `md`, with the semantic table retained at larger widths. The duplicate partial master-data strip was removed from Company, Branch, Location, Employee, and Supplier pages so the Sidebar/mobile More menu is the single module-navigation source. No schema or migration changed. |
| Adaptive Reports workspace | Replaced the single long Reports surface with four URL-backed, filter-preserving `overview`, `accounting`, `operations`, and `catalog` views while preserving the route, schema, APIs, RBAC, export permissions, and accounting formulas. The page now loads shared metrics plus only the active view, exposes removable filter chips with dependent Company/Branch clearing, preserves supported URL-only scopes when visible filters are applied, uses exact filtered data-quality/frequent-repair/cross-scope/`activity=idle_180d` drilldowns, restores browser-local presets to their saved view, handles blocked preset-storage access or writes without a client exception, separates module-owned export scope, localizes Thai/English copy and every supported filter value, aligns the route skeleton, and renders semantic desktop tables or labelled mobile cards for wide datasets. Mobile view navigation is a snap strip that reveals a directly loaded active tab. Final gates passed 1,088/1,088 tests, scoped ESLint, standalone TypeScript, a Next.js 16.2.4 build with Prisma 7.8.0 and 59/59 static pages, diff check, and an Impeccable `[]` result. Authenticated `system_admin` Chrome QA covered all four views at 375/390/414/768/1280/1440 with no horizontal overflow or sub-44px mobile actions; the first 375px Overview breakdown began at 5.9 viewport heights, and a direct 375px Catalog load revealed the active tab. Runtime logs emitted shared data plus only the matching active-view label, then timing was disabled. Permission-limited role, real-device, automated drilldown destination, and production-like Reports/export reconciliation remain explicit UAT gates. |
...
| Area | Update |
|---|---|
| Disposal bulk actual execution | Added controlled bulk execution to the approved queue for `disposal:edit`: explicit desktop/mobile selection mode, up to 20 requests of one disposal type, locked preview/commit shared values, read-only request-specific execution review, per-item serializable commit, partial results, stable unresolved retry, exhaustive localized errors, navigation discard protection, and Bangkok business date. The historical no-evidence exception appears only to `system_admin` when every selected request has zero effective evidence. Existing single execution, RBAC, SOD, evidence policy, movement/audit behavior, and request-specific values are preserved; no schema or migration was added. |
| Disposal historical-evidence exception | Documented the controlled, no-evidence execution exception for legacy disposals. Only `system_admin` may authorize it; approval, execution permission, SOD, disposal-type requirements, and final Disposed/Retired validation remain in force. Item and shared batch evidence reject exception mode, a 20-character historical reason plus acknowledgement is mandatory, normal work still requires evidence, and completed detail/print plus audit history preserve the grant. Bulk use is limited to approved selections where every request has zero effective evidence. The SQL migration is a manual production handoff that must wait for a fresh verified backup; migration and real-device/browser UAT have not been completed by this documentation update. |
...
| Area | Update |
|---|---|
| Asset Detail and Component Manager | Asset Detail now has one URL-backed tab row where each tab renders only its relevant information group, plus a permission-filtered header More menu and mobile lifecycle actions without duplicate edit/print/evidence/navigation controls. The existing custody/responsibility data and workflows are unchanged. Component installation/removal moved from Asset Detail/Edit into the scan-first `/[locale]/assets/{assetId}/components` manager. It reuses existing component APIs, audit logs, evidence, validation, and permissions; candidates require at least two search characters, view-only users cannot mutate, installation has identify/details/review steps and a visible success transition, removals use one in-app evidence dialog, and long history is progressively revealed. Detail initial reads now bound checkout and attachment previews while Evidence Center remains the complete evidence route. |
| Module UX follow-through | Completed the UI/UX roadmap without changing APIs, schemas, RBAC, SOD, audit history, or business workflows. Dashboard/Work Center now split overview from follow-up queues; Asset Detail has URL-backed reading presets and custody-operation review dialogs; Maintenance has table/board presentation; Disposal shows request stages and one permission-aware next action; Reports supports named browser-local filter presets; Settings searches keys/descriptions and links changed values to their tab; Audit Scan remembers a browser-local Location/Department work area per existing round for queue/progress presentation; and `ActionEmptyState` owns action spacing for empty/error/access-denied states. The initially shared partial master-data strip was later removed in favor of the complete Sidebar/mobile More navigation. Report presets remain device-local, and Audit work-area selection never creates a new round or changes scan/save behavior. |
| UI/UX hardening follow-up | General Asset Scan now preserves an allowlisted scanner return target through Asset Detail; Audit Scan clears stale scan feedback on target changes and announces result/queue updates; shared searchable selects provide keyboard navigation and focus return; confirmation prompts use the shared focus-managed dialog; My Assets has a server-scoped read-only self-service detail route; System Settings persists its tab in the URL and warns only for unsaved edits; Asset Detail and Reports gain shape-matched loading fallbacks; and visual tokens now consistently use Navy `#0F172A`, White, and Electric Blue `#3B82F6` for brand/accent roles while Action Blue `#2563EB` keeps white-text controls AA-readable. Shared `StatusPill` semantic tones replace Asset Detail's raw workflow colors and are reused by My Assets. No API, schema, RBAC, SOD, audit-log, scanner, or business workflow behavior changed. |
| Desktop Asset Register workspace | Asset Register now remembers browser-local filters, sort, and page size per locale when users return to the bare register route, while explicit query-string URLs remain authoritative and pagination is deliberately not restored. Existing column visibility persistence is retained. Detail navigation captures the dashboard content scroll position for the exact `returnTo` URL and restores it once on return. Bulk controls remain conditional on non-empty selection. The shared `ActionEmptyState` gained semantic empty/error/permission variants and now backs Asset Register zero-result states, dashboard errors, and access-denied pages without changing APIs, schema, RBAC, SOD, or asset workflows. Build compilation and TypeScript passed; production static page collection still requires SQL Server environment settings in the isolated worktree. |
| Mobile Field Navigation | Added a permission-aware mobile field dock for `หน้าหลัก / ทรัพย์สิน / สแกน / ตรวจนับ / เพิ่มเติม` while preserving the existing Sidebar as the full More menu. A tested route policy keeps Navigation Mode mutually exclusive with scanner, form, and contextual action bars; Audit Pending remains navigable while Audit Scan enters Focus Task Mode. The dock uses Lucide icons, visible labels, `aria-current`, 44px targets, safe-area spacing, virtual-keyboard hiding, RBAC destination filtering, and an employee My Assets fallback. Navigation Mode remains stable while the dock is transiently hidden for the More drawer or virtual keyboard, preventing Topbar reflow; More exposes the existing Sidebar with `aria-expanded` and `aria-controls`, moves focus to the drawer close button on open, restores it after close-button/overlay/Escape dismissal, and does not restore it when navigation begins from a drawer link. Controller QA passed at 375x812, 390x844, 768x1024, and 1024x800: Asset Register had one visible dock below `lg`, 1024px handed navigation to the 256px desktop Sidebar without a tablet gap, and selected Audit Scan had one Audit action bar with no field dock/no overflow. Manual Asset Tag/Serial/QR entry, `auditRoundId`, offline queue, RBAC, SOD, APIs, schema, and business workflows are unchanged. Latest checks passed the 14 focused mobile-navigation tests, `npm test` 677/677, scoped ESLint, and a 54/54-page production build. Real-device keyboard/rotation/safe-area, screen-reader focus, and permission-limited checks remain manual. |
| Adaptive UI accessibility findings | Fixed the final review's two verified Asset Register defects through TDD. RED was 24/30 with six expected failures; GREEN was 30/30. New semantic text tokens preserve the existing 10% badge backgrounds and labels while providing WCAG AA contrast: success `#166534` at 6.396:1, warning `#92400E` at 6.560:1, danger `#991B1B` at 7.112:1, and info `#1E40AF` at 7.572:1; muted remains 6.917:1. `StatusPill` now uses the foreground token classes. The mobile card's visible 20px checkbox is wrapped in a centered 44px semantic label with cursor and focus-within ring while preserving its aria label and selection logic. Scoped ESLint and standalone TypeScript passed; controller `npm run verify` passed 663/663 tests, zero lint errors with 256 existing bundled-tool warnings, Prisma 7.8.0 generation, Next.js 16.2.4 compilation/TypeScript, and 54/54 static pages. After a clean `http://localhost:3001` restart, controller browser QA passed at 375x812: `text-success-foreground` computed to `rgb(22, 101, 52)` / `#166534`, the checkbox measured 20x20 inside a 44x44 semantic label target, and body/card overflow were false. Ignored screenshot: `.superpowers/sdd/screenshots/task-6/asset-register-accessibility-final-375.png`. No API, schema, RBAC, SOD, business workflow, or dependency changed. |
| Adaptive UI Task 6 quality gate | Final release verification passed the 94/94 focused Task 2-5 suite, `npm run lint` (exit 0, 0 errors, 256 bundled `.agents`/`.gemini` warnings), `npm test` (656/656), `npm run build` (Prisma Client 7.8.0, Next.js 16.2.4, TypeScript complete, 54/54 static pages), `npm run verify`, standalone `npx tsc --noEmit`, and the pre-documentation `git diff --check` / clean `git status --short`. Package manifests/lock matched `HEAD`, `npm ls --depth=0` passed, and no dependency update was made; the earlier 13-vulnerability report (1 low, 8 moderate, 4 high) is deferred to a separate security review. Browser evidence was reviewed for App Shell 375/768/1280/1440, Asset Register 375/390/414/768/1280/1440 including filtered-empty/disclosure, and Audit Scan 375/390/414/768/1280/1440 including selected-action/safe-area states. Real-device Android/iPhone camera, torch/zoom, offline-photo, keyboard, rotation, safe-area, and permission-limited Asset role checks remain manual; the in-app browser did not receive a video stream and is not a camera pass. The raw scan found expected globals/docs/tests plus two inherited Asset Detail component lines with raw success/action colors; Asset Detail was outside Tasks 2-5, so it and the known stale `scanFeedback` sequence remain separately documented. No API, schema, RBAC, SOD, audit workflow, camera runtime, or print behavior changed. |
| Adaptive Audit Scan controller QA | Reviewer-approved browser QA observed one primary and one supporting region and no body overflow at 375/390/414/768/1280/1440. The 375px Recent Scans header was readable at 284px by 86px; 1280 used a same-node two-column form grid. Initial mobile state had no action bar; selected in-round items showed exactly one mobile action region below 768px with matched, mismatch/Finding, and change/rescan text only, while the hook was hidden at 768px and above. Mobile content reserved 156px, desktop reserved 0, and controller safe-area padding computed to 12px. The in-app browser created one `#audit-qr-reader` but received no stream/permission, so camera/torch/zoom and Android/iPhone offline-photo/keyboard/rotation/safe-area checks remain manual. Reviewer verification recorded 158 focused tests, scoped ESLint, TypeScript, production build, and diff check passing. Known low-priority debt, not a Task 5 regression: `scanFeedback` remains visible after an out-of-scope lookup followed by selecting an in-round pending item. |
| Adaptive Audit Scan workspace | Consolidated the existing Audit Scan into one adaptive workspace without changing APIs, payloads, schema, RBAC, SOD, camera decoder/constraints, target lock, recent-scan edit, component workflow, evidence requirements, offline queue, audit history, findings, or result semantics. Mobile now gives the sticky progress strip a scanner-first primary sequence, places fallback/evidence/recent/pending/offline work after it, and shows the fixed matched/mismatch/change-target bar only for an in-round target. The scan keeps one `audit-qr-reader` subtree, controls remain by the preview, and safe-area padding uses `max(0.75rem, env(safe-area-inset-bottom))` with content reservation. Controller browser QA is complete; real-device browser QA remains open. |
...
| Enterprise navigation shell | Refreshed the existing dashboard presentation with the committed semantic tokens: a Brand Navy desktop/mobile drawer, Sidebar Foreground/Hover/Active navigation states, full-row active treatment without a right stripe, and Electric Blue focus rings. The topbar remains Surface White with Border Slate separation; labels, Lucide icons, hrefs, RBAC filtering, nesting, collapse, overlay, language, notification, user-menu, and responsive behavior are unchanged. Browser QA passed at 375x812, 768x1024, 1280x800, and 1440x900: mobile/tablet drawers remained off-canvas until opened, Thai labels fit, desktop 256px and collapsed 64px sidebar states aligned with topbar/main, nested Asset navigation expanded, and no body overflow occurred. The Navy `#0F172A` sidebar, white topbar, blue active state, and readable text were verified; ignored screenshots are under `.superpowers/sdd/screenshots/task-3`. |
| Adaptive Asset Register | Preserved the existing URL-backed filters, sorting, pagination, column presets, bulk selection/update, exports, import placement, permissions, and `returnTo` navigation while making responsive responsibilities explicit. Final `system_admin` browser QA passed: at 375/390/414 the 25-card mobile list had no body/list/card horizontal overflow, readable text status, the intended hierarchy, omitted company/price, a working full-width 44px secondary disclosure, and a filtered-empty `ไม่พบข้อมูล` state without overflow; at 768/1280/1440 only the dense desktop table rendered without body overflow. Mobile search precedes quick filters (top 249 versus 568 at 375), while desktop preserves quick filters before the form (top 185 versus 447 at 1280). Runtime QA caught and fixed the missing canonical status/condition projection; semantic mappings are explicit and exhaustively tested. Reviewer approved after 651/651 full tests, 35/35 focused tests, scoped ESLint, `tsc --noEmit`, and a production build. Permission-limited role and unobserved loading/navigation states remain manual. |
...
| Audit round cancellation | Audit rounds can now be cancelled with an auditable reason. Cancellation preserves audit items, scan history, findings, evidence, logs, and already-applied asset corrections without rollback, makes the round read-only, and removes it from operational queues/current coverage while keeping historical detail visible with cancellation time, user, and reason. |
...
| Area | Update |
|---|---|
| Integration API Phase 1 | Added the read-only external Integration API foundation under `/api/integrations/v1`: hashed Bearer-token clients, scoped auth helpers, request IDs, audit-log summaries, custom route-inventory classification, and `GET /api/integrations/v1/health` for token validation. Existing UI/session APIs and scheduler tokens remain separate. |
...
| Integration API Phase 3 | Added `GET /api/integrations/v1/assets/changes?updatedSince=...` for bounded incremental asset sync with stable cursor/high-water metadata, plus read-only `reference:read` endpoints for statuses, companies, branches, and locations. Reference DTOs expose compact operational code/name metadata only, and all new routes audit request summaries without logging tokens or response payloads. |
| Integration API Phase 4 | Added authenticated `GET /api/integrations/v1/openapi` under `integration:read`, local `npm run integration:token` tooling to generate a plain token plus SHA-256 hash without writing secrets to disk, and `docs/13_INTEGRATION_API.md` covering configuration, scopes, endpoint examples, UAT, rotation, and production notes. |
| Integration API DB Token Manager | Replaced active token lifecycle configuration with DB-backed clients in `integration_api_clients`, managed through `Admin > Integration API` and `/api/admin/integration-clients` for create, edit display name/scopes, rotate, disable, and enable. Plain tokens are returned once on create/rotate; the app stores only `tokenHash` and `tokenPreview`. Scope expansion requires confirmation because existing tokens gain the newly allowed endpoint immediately, and client changes are audited with safe old/new values. Production must apply `prisma/manual-migrations/2026-06-14-add-integration-api-clients.sql` after backup/approval before deploy/use, while existing database connection settings remain unchanged. `npm run integration:token` remains recovery/troubleshooting tooling only. Emergency disable uses the admin UI or controlled SQL against `integration_api_clients.enabled`; code rollback is required before removing the table. |
...
| Area | Update |
|---|---|
| Asset Detail evidence drawer | Asset Detail no longer renders the cross-workflow Evidence Center as a duplicate gallery section. The top action row now opens a drawer-based evidence file index with counts and source filters, while `รูปทรัพย์สิน` stays focused on register photos/files and `Timeline รวมเหตุการณ์` remains the primary event-history surface. |
...
| Area | Update |
|---|---|
| Asset Register lifecycle quick filters | `/th/assets` lifecycle quick filters now include In Use (`ใช้งานอยู่`) and Checked Out (`ถูกเบิก`) alongside Ready, Pending Repair, and Under Maintenance, using the existing `asset_statuses` rows and preserving the current search/company/branch/category context. |
| Asset Register table polish | `/th/assets` now consolidates active search/filter/drilldown criteria into one removable summary with a clear-all action, keeps Asset Tag/name columns sticky on the left and row actions sticky on the right for wide desktop tables, shows a horizontal-scroll affordance, clamps long asset names to two readable lines, and adds a visible keyboard focus ring for clickable rows. |
| Asset Register workflow UX | `/th/assets` now groups URL-backed quick filters by data quality, ownership/custody scope, and lifecycle status, collapses advanced filters unless active, moves the Import Wizard below the table/card list, defaults the desktop table to the Operations column preset before stored preferences load, and hides column/export/template controls on mobile. Search, drilldown filters, exports, and existing Asset Register business logic remain unchanged. |
...
| Audit Scan queue focus UX | `/th/audit/rounds/{id}/scan` now highlights the scan/manual entry panel before an asset is selected and adds a sticky-header `คิวรอตรวจ {count}` action. The queue opens an inline preview from existing `audit_items` for the round and links to the existing pending list for not-found handling. No audit workflow, scan history, finding, permission, SOD, or offline queue logic changed. |
| Audit Scan field device UX | `/th/audit/rounds/{id}/scan` now uses a compact Fast Scan Mode toggle and combines the active scan-result card with previous QR/manual scan outcomes for the current page session. This removes duplicated large `ผลสแกน` / `สแกนล่าสุด` sections while keeping clear matched, mismatch, out-of-scope, unknown, found-later, saved, and offline-queued labels. Supported mobile camera tracks expose a flashlight toggle inside the scanner preview; unsupported browsers/devices hide the control and torch is turned off on scanner stop/camera switch. Audit workflow, scan history, findings, offline queue, permissions, and SOD behavior are unchanged. |
...
| Area | Update |
|---|---|
| Cross-scope asset visibility | Asset Register quick filters now expose assets whose custodian company, custodian branch, or current location branch differs from the asset owner/tag scope. Dashboard adds a cross-scope KPI, urgent-work card, and review preview with drilldowns back to Asset Register. Reports adds the same cross-scope summary and preview, Asset Register export adds owner/custodian/home/current location branch columns and human-readable flags, and Asset Overview export adds cross-scope metrics plus a Cross Scope sheet. |
| Asset Form save UX | Single asset create/edit now keeps Save and Cancel in a fixed bottom action bar so operators can save long forms without scrolling to the final section. The bar keeps the existing duplicate, protected-status, and saving disabled guards and leaves enough bottom spacing for the final fields. |
...
| Legacy asset check-in | `/th/asset-management/checkin` now shows imported/current-custodian assets without active checkouts in a searchable legacy-data panel. Operators can filter by Asset Tag, asset name, current holder, or location, create an auditable `legacy_return_backfill` checkout, and then continue through the standard return workflow. |
...
> **Latest Update:** ✅ Asset Register workflow UX now groups quick filters, collapses advanced filters, prioritizes the table/card list before the Import Wizard, defaults the table to the Operations preset, and keeps mobile focused on compact asset cards. Cross-scope asset visibility Phase 1 remains reporting/listing only and leaves custody, transfer, and asset master workflows unchanged.
...
- **Category Soft Delete / Reactivation** `AssetCategory.code` ยัง unique ทั้ง active/inactive rows ตาม SQL Server constraint; ถ้าสร้างหมวดหมู่ด้วย code เดิมที่ inactive ระบบจะ reactivate row เดิมแทนการ insert ใหม่, block การ delete/deactivate เมื่อมี asset/model อ้างอิง, ยังอนุญาตให้แก้ custom-field template ของ category ที่ active อยู่, และแยก nested write payload ระหว่าง create-only กับ update/reactivate replace เพื่อไม่ส่ง `deleteMany` เข้า `assetCategory.create()`
...
- **Asset/Model Photos** ใช้ตาราง `attachments` เดิม: `module=asset` สำหรับรูปทรัพย์สินจริง, `module=asset_model` สำหรับรูปกลางของรุ่น, ไฟล์อยู่ใต้ `UPLOAD_DIR`; หน้า Asset Register ใช้รูปประจำรุ่นเป็น thumbnail หลักก่อนแล้วค่อย fallback เป็นรูปทรัพย์สินจริงเมื่อรุ่นไม่มีรูป, หน้า Brand/Model แสดง thumbnail รุ่นในคอลัมน์ชื่อรุ่นโดยไม่เพิ่มคอลัมน์ใหม่ และ preview รูปใช้ `object-contain` เพื่อไม่ตัดรูป
...
- **Mobile Asset QR / Serial Scanner Frame** `ScannerTextInput` และ Audit Scan แยกโหมด Asset Label QR ออกจาก Serial Number barcode scan ชัดขึ้น: `/asset-management/scan` และ `/audit/rounds/{id}/scan` ใช้กล้องพื้นผิว square พร้อม custom square guidance overlay, บังคับ Asset QR ไปที่ ZXing decoder ผ่าน native-resolution video frame แทน CSS-pixel hidden canvas ของ `html5-qrcode`, ไม่ใช้ native `BarcodeDetector` และปิด mirror-flip retry, และไม่ใช้ CSS `object-cover` กับวิดีโอเพื่อลดโอกาสที่ภาพที่เห็นกับพื้นที่ decode จะคลาดกัน ส่วนช่อง Serial Number ใช้ native-resolution video path เดียวกันกับ ZXing `BrowserMultiFormatReader`, optional browser `BarcodeDetector`, focused scan-band canvas crops, QR/1D/2D barcode hints, camera constraints 1920x1080, no forced serial digital zoom, และ 16:9 scan-line guide เพื่ออ่าน barcode บน manufacturer label ได้ชัดขึ้น
...
- **Asset Edit Custodian Scope** หน้าแก้ไขทรัพย์สินจะเปิดโหมดผู้ถือครองต่างบริษัท/นอก owner scope ให้อัตโนมัติเมื่อ `custodianId` เดิมไม่อยู่ในรายการที่ถูกกรองตามบริษัท/สาขา/แผนกเจ้าของทรัพย์สิน ทำให้ผู้ถือครองเดิมแสดงในช่องเลือกทันทีแทนการดูเหมือนว่างจนกว่าจะติ๊ก checkbox เอง
- **Asset Cross-Branch Location Scope** หน้าเพิ่ม/แก้ไขทรัพย์สินและเพิ่มแบบชุดแยก owner/tag scope ออกจาก physical location scope: location dropdown ยังจำกัดตามสาขาเจ้าของรหัสเป็นค่าเริ่มต้น แต่เปิด `แสดงที่ตั้งต่างบริษัท/ต่างสาขา` เพื่อเลือกที่ตั้งประจำ/ที่ตั้งปัจจุบันคนละสาขาได้โดยไม่เปลี่ยน Asset Tag scope; เมื่อเปิดโหมดนี้ option จะแสดง label สาขากำกับเพื่อแยกรหัสพื้นที่ซ้ำ และแสดง warning หาก location ที่เลือกอยู่นอก owner branch
...
- **Audit Rounds Action-First UX** หน้า `/audit/rounds` เพิ่ม next-action cards สำหรับสแกนต่อ, Review รายการไม่ตรง, และรอบที่พร้อมปิด พร้อม quick filters ผ่าน query `view=all|open|pending|review|mismatch|readyToClose` ที่ไม่รีเซ็ต dashboard scroll position เมื่อสลับตัวกรอง และ badge รายรอบว่าพร้อมปิดหรือยังติด pending/review โดยยังใช้ logic scan, finding review, SOD, และ close-round guard เดิม
...
- **Reports Branch Label Disambiguation** หน้า `/reports` แสดง breakdown ตามสาขาเป็น `Company / Branch - Name` และ shared report table ใช้ key ที่ไม่อิง label ล้วน เพื่อกัน React duplicate-key warning เมื่อมีสาขาคนละบริษัทที่ชื่อ/รหัสซ้ำกัน
...
- **Asset Relationship Map UI polish** หน้า Asset Detail ปรับแผนภาพความสัมพันธ์เป็น 3 lane (`เป็นส่วนควบของ` / รายการที่กำลังดู / `มีส่วนควบ`) พร้อม status summary, empty state แยกซ้าย/ขวา, note ด้านล่าง และ relationship card ที่ให้ Asset Tag แสดงเต็มก่อนชื่อทรัพย์สินกับ badge เพื่ออ่านเลขทรัพย์สินได้ชัดเจน
...
| **QR scanner integration** | หน้า `/audit/rounds/{id}/scan` รองรับกล้องผ่าน native-resolution ZXing Asset QR decoder และ fallback paste URL/Asset ID/Asset Tag; QR label URL `/assets/{id}` และ `/q/a/{assetId}` จะ map กลับ audit item ได้ |
...
| **Reports shared filters** | หน้า `/reports` เพิ่ม filter panel กลางสำหรับ search, company, branch, category, status, และ condition; ตัวเลข overview, data quality counters, group tables, link ไปทะเบียนทรัพย์สิน, `/api/assets/export`, และ `/api/reports/assets-overview/export` ใช้ query filter ชุดเดียวกัน; breakdown ตามสาขาแสดง `Company / Branch - Name` เพื่อแยกสาขาชื่อซ้ำต่างบริษัท และ report table ใช้ key ที่ไม่ชนเมื่อ label ซ้ำ |
...
| **Lifecycle status guidance and repair validation** | เพิ่ม diagram `docs/asset-lifecycle-flow.png`/`.svg`, อธิบายความต่างระหว่างสถานะ lifecycle กับสภาพทรัพย์สิน, บล็อก Asset Form ไม่ให้เปลี่ยน protected lifecycle statuses โดยตรง และปรับ validation ใบซ่อมให้เปิด ticket ได้โดยไม่ต้องส่ง `returnDate` ส่วนวันที่คืนยังบังคับตอนปิดใบซ่อม |
...
| **Asset Form Serial scanning** | หน้า `/assets/new` และ `/assets/{id}/edit` ใช้ `ScannerTextInput` กับช่อง Serial Number เพื่อเปิดกล้องมือถือสแกน QR/barcode ผ่าน native `getUserMedia` + ZXing `BrowserMultiFormatReader`; รองรับ QR, Code 128/39/93, EAN/UPC, ITF, Codabar, Data Matrix, PDF417, เพิ่ม optional browser `BarcodeDetector` และ focused scan-band crops ที่ถอดรหัส crop `ImageData` โดยตรงผ่าน ZXing luminance/binarizer primitives ก่อน fallback ไป full frame, ใช้ preview 16:9 พร้อม scan-line guide สำหรับ barcode, auto-fill แล้วหยุดสแกนทันที โดยยังแก้ไขค่าด้วยมือก่อนบันทึกได้ |
...
184. Mobile Asset QR scanner frame with square `/asset-management/scan` and Audit Scan camera surfaces, custom square guidance overlay, native-resolution ZXing Asset QR decoding from the video frame instead of `html5-qrcode` CSS-pixel canvas capture, mirror-flip retries disabled, no CSS video `object-cover` in Asset QR mode, and regression coverage preserving wider Serial Number barcode preview; Audit Scan QR reads are now target-locked after success as noted in the 2026-06-26 handoff
185. Serial Number barcode scanner fallback with optional browser `BarcodeDetector`, focused scan-band crops before full-frame ZXing decode, no forced serial digital zoom, narrower 16:9 scan-line guidance overlay, updated Thai/English help text, and regression tests for the serial decode path
186. Serial Number crop fallback now decodes crop `ImageData` directly through ZXing `MultiFormatReader`, `RGBLuminanceSource`, and `BinaryBitmap` instead of passing a canvas into `BrowserMultiFormatReader`, fixing mobile browser cases where the bundled browser reader did not decode focused crop canvases reliably
187. Category create custom-field nested write guard splits create-only and replace payload helpers so `assetCategory.create()` never receives `deleteMany`, with regression coverage for adding categories that have an empty template list
188. Audit Scan field-workflow UX with a system-data comparison panel, `ข้อมูลตรง`/`ข้อมูลไม่ตรง` fast mode actions, mismatch photo requirement when actual fields differ, free-form multiple evidence photo queueing, optional evidence tags, hydration-safe camera readiness initialization, and regression coverage for the new field-mode behavior
...
191. Audit Scan result semantics and mobile Phase 2 layout: successful scans now present `บันทึกพบตรง`, `บันทึกข้อมูลไม่ตรง / Finding`, and the current `เปลี่ยน / สแกนใหม่` target-change action instead of a primary Not Found action; out-of-round assets are Out-of-scope, unregistered QR values are Unknown Asset, Found Later uses the existing recovery logic, and mobile scan pages use a compact sticky progress header plus a bottom post-scan action bar without changing audit business logic
...
194. Audit out-of-scope actual field capture with prefilled master values, evidence-required changed fields linked to audit finding attachments, structured field-specific findings for wrong location/custodian/department/condition, review-confirmed out-of-scope findings without direct master updates, and regression coverage preserving review-controlled master asset changes
195. Audit component missing and finding evidence visibility: component `ไม่พบ` actions now use an in-app mobile dialog instead of `window.prompt`, allow optional remark/photo evidence through multipart `mark-not-found`, attach evidence to `audit_finding`, and `/audit/findings` loads those attachments as inline openable evidence links for reviewers; regression tests cover copy, dialog shape, attachment queries, and unsupported regex target compatibility.
