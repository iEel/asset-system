...
- **Audit Rounds Action-First UX** หน้า `/audit/rounds` เพิ่ม next-action cards สำหรับสแกนต่อ, Review รายการไม่ตรง, และรอบที่พร้อมปิด พร้อม quick filters ผ่าน query `view=all|open|pending|review|mismatch|readyToClose` และ badge รายรอบว่าพร้อมปิดหรือยังติด pending/review โดยยังใช้ logic scan, finding review, SOD, และ close-round guard เดิม
