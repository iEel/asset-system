...
184. Mobile Asset QR scanner frame with square `/asset-management/scan` and Audit Scan camera surfaces, custom square guidance overlay, native-resolution ZXing Asset QR decoding from the video frame instead of `html5-qrcode` CSS-pixel canvas capture, mirror-flip retries disabled, no CSS video `object-cover` in Asset QR mode, continuous audit scan support, and regression coverage preserving wider Serial Number barcode preview
