...
> **Latest Update:** ✅ Category master data create now separates the Prisma nested custom-field payload used for new rows from the replace payload used by update/reactivation, so creating a category with an empty template no longer sends `deleteMany` into `assetCategory.create()`. Asset Label QR scanning on `/asset-management/scan` and Audit Scan default to the generic rear camera selector, use an undistorted 4:3 mobile camera preview, and decode directly from the native-resolution video frame through ZXing. Serial Number scanner inputs use native-resolution ZXing `BrowserMultiFormatReader`, optional browser `BarcodeDetector`, focused scan-band crop `ImageData` fallback, and a narrower 16:9 scan-line guide for sharper manufacturer barcode capture.
