...
> **Latest Update:** ✅ Audit Findings now has a Phase 1 Findings Resolution Center on `/audit/findings`, with pending review/open action/overdue/closed queues, system-vs-found comparison rows, resolution-state badges, and filtered Excel/PDF exports on the existing review workflow. Audit Rounds remains action-first for high-volume counting, and Audit Scan keeps the field decision flow with optional matched photos and required mismatch evidence. Asset Register detail/edit/clone navigation now preserves the originating list page, filters, sort, and search through a sanitized return target.
