...
- [ ] On Asset Register at 375, 390, and 414 pixels, confirm cards only: Asset Tag plus readable semantic status, name, serial/category metadata, location, custodian, applicable condition/shared-ownership warnings, then detail/edit actions. Confirm company/branch and purchase price are not in the default mobile card, clone/delete stay secondary, long Thai/asset values do not overlap, and the body does not overflow horizontally.
- [ ] On Asset Register at 768, 1280, and 1440 pixels, confirm the dense desktop table only is visible, search/company/branch/category and existing utility workflows remain intact, and empty, filtered-empty, loading/navigation, and permission-limited states retain the same operational visual vocabulary.
