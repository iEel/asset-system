...
- [ ] Desktop 1440x900: enter one optional shared approval remark, commit, and confirm the remark appears only on successful requests while each request preserves its sale/salvage values, evidence, disposal type, own approver/timestamp, movement, audit record, and parent batch status. Confirm there is no bulk reject or bulk execution action.
