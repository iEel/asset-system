...
- [ ] Tap `คิวรอตรวจ {count}` from the sticky header and confirm an inline pending queue preview opens from the same audit round. Selecting an item should load it for review, while marking not found still happens from the full pending list/zone queue.
