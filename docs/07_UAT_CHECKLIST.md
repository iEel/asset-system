...
- [ ] On a mobile device, open fullscreen walking mode and confirm the audit remains in the same round while the screen switches to a compact walking-scan surface.
- [ ] In fullscreen walking mode, scan or select an item and confirm the scan result appears in a bottom sheet with `บันทึกพบตรง`, `บันทึกข้อมูลไม่ตรง / Finding`, `ถ่ายรูปหลักฐาน`, and `สแกนต่อ / กรอกเอง`.
...
- [ ] Tap `คิวรอตรวจ {count}` from the sticky header and confirm an inline pending queue preview opens from the same audit round. In fullscreen walking mode, confirm the queue is grouped by expected location. Selecting an item should load it for review, while marking not found still happens from the full pending list/zone queue.
...
- [ ] On a mobile browser that supports vibration, confirm scan result feedback gives a light vibration cue; on unsupported devices, confirm the scan flow still works without any error.
- [ ] Confirm Fast Scan Mode appears as a compact inline toggle and the old large blue help block is not repeated.
- [ ] Confirm the post-scan bottom action bar appears only after an asset is selected or scanned.
- [ ] Scan or manually enter several audit items and confirm the current `ผลสแกน` and previous `สแกนล่าสุด` entries appear in one combined result panel with readable matched/mismatch/out-of-scope/unknown/found-later/offline-queued labels without replacing the saved audit history.
