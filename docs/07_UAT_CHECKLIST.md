...
- [ ] Without scanning, open the manual item picker and confirm it is collapsed by default, searchable by Asset Tag/name/location/custodian/department, and does not show the old long native asset dropdown.
...
- [ ] Confirm Fast Scan Mode and Continuous Scan appear as compact switch rows below the scan input, expose proper on/off state, and do not repeat the old large blue help blocks on mobile.
