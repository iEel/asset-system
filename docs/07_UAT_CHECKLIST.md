...
- [ ] Scan or manually enter several audit items and confirm the current `ผลสแกน` appears first and previous `สแกนล่าสุด` entries are available in a collapsible section with readable matched/mismatch/out-of-scope/unknown/found-later/offline-queued labels without replacing the saved audit history.
