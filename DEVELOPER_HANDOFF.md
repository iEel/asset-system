...
> **Latest Update:** ✅ RBAC API route inventory coverage added. `tests/rbac-route-matrix.test.ts` now scans every `src/app/api/**/route.ts` file and requires each route to be covered by the RBAC matrix, a normal `requireAuth` guard, custom authorization, or a documented public exception.
