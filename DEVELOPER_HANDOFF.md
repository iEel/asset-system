...
> **Latest Update:** ✅ Deployment guide now documents PM auto-generation through a dedicated `systemd timer` + oneshot service, including local-app execution, scheduler token setup, dry-run/manual testing, journal log checks, and production checklist coverage.
...
- **Audit Scan Offline/Resume Queue** ใช้ `src/lib/audit-offline-queue.ts` เก็บ scan payload ที่ส่ง `/api/audit-rounds/{id}/scan` ไม่สำเร็จไว้ใน `localStorage` แยกตามรอบตรวจ; หน้า scan แสดง banner จำนวนรายการค้างและปุ่ม retry เมื่อกลับมาออนไลน์ โดยจงใจไม่ persist ไฟล์รูปใน queue ข้าม reload และจะแจ้งเตือนผู้ใช้เมื่อมีรูปค้าง
...
- **Accounting / Depreciation View** หน้า `/reports` ใช้ `src/lib/asset-depreciation.ts` คำนวณค่าเสื่อมแบบเส้นตรงจาก `purchasePrice` + `purchaseDate` ตามตัวกรองปัจจุบัน โดย default useful life คือ 60 เดือน และลดเหลือ 36 เดือนสำหรับ software/license และ component; แสดงต้นทุนที่คิดค่าเสื่อม ค่าเสื่อมสะสม มูลค่าตามบัญชีสุทธิ รายการขาดข้อมูลบัญชี และส่งออก sheet `Depreciation` ใน Asset Overview Excel
- **Design System Primitives** เพิ่ม `MetricCard`, `ContentPanel`, และ `src/lib/design-system.ts` เพื่อรวม card/panel/tone class ที่ใช้ซ้ำในรายงานและหน้า admin; `/reports` เริ่มใช้ primitive กลางแทน local metric card แล้ว พร้อม test `tests/design-system.test.ts` สำหรับ tone class stability
...
- **Preventive Maintenance auto-generation** เพิ่ม endpoint `POST /api/maintenance-plans/generate-due` สำหรับ manual หรือ scheduler token (`MAINTENANCE_PM_GENERATION_TOKEN`) และ script `npm run pm:generate-due`; job ค้นหาแผน active ที่ `nextDueDate` ถึงวันนี้, กันใบงานซ้ำด้วย open `[PM] {planNo}` ticket guard, ข้ามแผนที่ไม่มี reporter, สร้างใบงานซ่อมจริง, เลื่อน `nextDueDate`, และบันทึก audit summary แบบ batch
- **PM production scheduler guide** `DEPLOYMENT_UBUNTU_CLOUDFLARE.md` แนะนำให้ใช้ `asset-system-pm-generate.service` + `asset-system-pm-generate.timer` แทน crontab สำหรับ PM อัตโนมัติ โดย service เป็น `Type=oneshot`, โหลด `/var/www/asset-system/env/asset-system.env`, override `AUTH_URL/NEXTAUTH_URL` ไป `http://127.0.0.1:3000` เฉพาะ job หลังบ้าน, ใช้ `MAINTENANCE_PM_GENERATION_TOKEN`, มีคำสั่ง dry-run/manual start และตรวจ log ผ่าน `journalctl`
...
| **Accounting / Depreciation view** | หน้า `/reports` เพิ่มมุมมองบัญชีแบบ read-only คำนวณค่าเสื่อมแบบเส้นตรงจากราคาซื้อ/วันที่ซื้อ แสดง accumulated depreciation, net book value, fully depreciated count, missing accounting info และเพิ่ม sheet `Depreciation` ใน Asset Overview Excel |
| **Design system consolidation** | เพิ่ม `ContentPanel` และ `MetricCard` กลางสำหรับ section/KPI card ที่ใช้ซ้ำ พร้อม `src/lib/design-system.ts` สำหรับ tone class ที่ทดสอบได้; หน้า Reports เริ่มใช้ primitive นี้เพื่อลด local component/style drift |
...
| **File Storage Governance** | เพิ่มหน้า `/admin/storage` ใต้เมนู Administration และ helper `src/lib/storage-governance.ts` สำหรับสรุปไฟล์แนบ active/inactive, total storage, ไฟล์ขนาดใหญ่, module breakdown, path ซ้ำ และ path ว่าง โดยเป็น read-only governance ก่อนตัดสินใจ archive/delete จริง |
...
151. File Storage Governance with `/admin/storage`, attachment module breakdown, large-file and duplicate/missing-path review lists, sidebar navigation, and helper regression tests
...
153. Audit Scan Offline/Resume Queue with per-round localStorage persistence for failed scan submissions, visible queued-count retry UI, post-success queue removal, and helper regression tests
154. Accounting / Depreciation read-only view with straight-line depreciation helper, Reports net book value metrics/table, Asset Overview Excel depreciation sheet, and helper regression tests
155. UI/UX Design System Consolidation with shared `MetricCard`, `ContentPanel`, tested tone helpers, and Reports refactor away from local metric card styling
...
158. Preventive Maintenance auto-generation with `POST /api/maintenance-plans/generate-due`, `npm run pm:generate-due`, scheduler bearer token `MAINTENANCE_PM_GENERATION_TOKEN`, due-plan scan, open-ticket duplicate guard, missing-reporter skip path, next-due-date progression, batch audit logging, and RBAC regression coverage
...
161. Asset Import Batch/Staging v1 with preview-generated batch IDs, batch readiness/mapping summaries in the import wizard, confirm-time batchId reuse, per-asset import log stamping, batch-level audit trail entries, and helper regression tests
