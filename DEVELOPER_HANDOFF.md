...
- For mobile camera scanning, test `/th/asset-management/scan`, Serial Number scanner inputs, and `/th/audit/rounds/{id}/scan` on the target phone browser. Some browsers hide camera labels until permission is granted, so the app exposes a generic rear-camera option plus any named device list returned by the browser. `/th/asset-management/scan` intentionally scans Asset Label QR codes only; use the Serial Number fields when collecting manufacturer barcodes.
