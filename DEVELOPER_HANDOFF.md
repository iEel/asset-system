...
1. **Asset Register duplicate UX** — surface duplicate serial conflicts inline before submit where practical
2. **Master data table scaling** — server-side pagination/sort for high-volume master data modules
3. **Operations phase kickoff** — check-out/check-in flow once Asset Register polish is accepted
