...
> **Latest Update:** ✅ PWA install prompt UX added: Topbar now shows a compact install button when the browser exposes `beforeinstallprompt`, and iOS/iPadOS users get Add to Home Screen guidance without a blocking banner.
...
- **Audit Scan Offline/Resume Queue v2** ใช้ `src/lib/audit-offline-queue.ts` เก็บ scan payload ที่ส่ง `/api/audit-rounds/{id}/scan` ไม่สำเร็จไว้ใน IndexedDB แยกตามรอบตรวจ พร้อม photo blobs, sync status, และ last sync error; ถ้า IndexedDB ใช้ไม่ได้จะ fallback เป็น `localStorage` สำหรับ payload เดิม หน้า scan แสดง banner จำนวนรายการค้างและปุ่ม retry เมื่อกลับมาออนไลน์
