...
- Master Data usability now presents the Brand / Model page as a left-side brand navigator plus a right-side model workspace. Create Model is the primary header action, Create Brand remains available, duplicate review is compacted, and brand navigator counts are derived from active `AssetModel`/`Asset` group counts so soft-deleted models do not make the sidebar disagree with the visible model table. Supplier master data labels the existing `Supplier.code` field as `Tax ID / Supplier Code` (`เลขประจำตัวผู้เสียภาษี / รหัสผู้ขาย`) for Thai vendor workflows while keeping legacy/internal vendor code compatibility and the existing 20-character unique column.
