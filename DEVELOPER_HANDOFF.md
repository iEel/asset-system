...
- Production readiness now tracks PM, LDAP sync, and Notification Digest scheduler run status separately. Upload hardening now includes content signature validation and an optional server-side scanner hook in addition to size, MIME, and extension checks.
