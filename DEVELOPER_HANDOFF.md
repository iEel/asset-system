...
| **Lint/build verification** | ล่าสุด `node --test tests\approval-permission-matrix.test.ts tests\approval-inbox.test.ts` และ `npm run build` ผ่านหลังเพิ่ม Approver Permission Matrix; unauthenticated HTTP smoke ของ admin routes redirect/login ตาม session guard จึงใช้ build + helper tests เป็นหลัก |
