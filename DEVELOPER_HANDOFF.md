...
| **Lint/build verification** | ล่าสุด `node --test tests\work-center-view.test.ts tests\asset-data-quality-filter.test.ts tests\work-center-metrics.test.ts`, `node --test tests\*.test.ts` (36 tests pass), `npm run build`, และ Browser QA ที่ `/th/work-center?panel=assets` ผ่านหลัง Work Center upgrade; unauthenticated HTTP smoke ของ admin routes redirect/login ตาม session guard จึงใช้ build + helper tests เป็นหลัก |
