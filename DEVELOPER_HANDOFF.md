...
| **Lint/build verification** | ล่าสุด `node --test tests\approval-decision-log.test.ts tests\system-log-presenter.test.ts tests\approval-inbox.test.ts` และ `npm run build` ผ่านหลังเพิ่ม Approval Decision History; unauthenticated HTTP smoke ของ admin routes redirect/login ตาม session guard จึงใช้ build + helper tests เป็นหลัก |
