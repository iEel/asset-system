...
| `src/components/maintenance/maintenance-plan-form.tsx` | Collapsible preventive maintenance plan create form on the Maintenance page |
| `src/lib/preventive-maintenance.ts` | Preventive maintenance frequency, due-state, and summary helper |
...
| **Preventive Maintenance plans** | เพิ่ม schema/table `maintenance_plans`, helper `src/lib/preventive-maintenance.ts`, API `GET/POST /api/maintenance-plans`, และแผง PM บนหน้า `/maintenance` สำหรับดู overdue/due soon/upcoming, รายการแผนล่าสุด, และฟอร์มเพิ่มแผน PM โดยยังแยกจากใบซ่อมจริงใน `maintenance_tickets` |
