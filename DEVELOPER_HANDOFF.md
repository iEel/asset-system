...
> **Latest Update:** ✅ Notification Daily Digest v1 added. A scheduler endpoint and `npm run notifications:digest` now deliver daily in-app digest notifications per active user, with duplicate guard, dry-run support, audit logging, and digest history on the Notification Center page.
