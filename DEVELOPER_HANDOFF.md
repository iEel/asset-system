...
| **Lint/build verification** | ล่าสุด `node --test tests\*.test.ts` และ `npm run build` ผ่านหลังเพิ่ม Central Approval Inbox v1; Browser plugin ในเครื่องนี้ไม่มี `scripts/browser-client.mjs` จึงใช้ in-app browser verify ไม่ได้ |
