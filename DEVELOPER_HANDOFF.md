...
| **Lint/build verification** | ล่าสุด `node --test tests\*.test.ts` และ `npm run build` ผ่านหลัง refactor Approval Inbox query/count helper กลาง; Browser plugin ในเครื่องนี้ไม่มี `scripts/browser-client.mjs` จึงใช้ in-app browser verify ไม่ได้ |
