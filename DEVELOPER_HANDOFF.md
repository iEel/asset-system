...
> **Latest Update:** ✅ RBAC Route Regression Matrix added. `src/lib/rbac-route-matrix.ts` documents critical API route permission expectations and `tests/rbac-route-matrix.test.ts` reads real route files to catch missing `requireAuth` or permission snippets.
