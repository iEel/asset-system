...
- Asset Register now supports high-volume operational views directly on `/th/assets`: URL-backed quick filters cover all assets, missing Serial, missing photo, purchase-info gaps, warranty expiring, missing responsibility, Ready, Pending Repair, and Under Maintenance while preserving the existing search/company/branch/category context. The register search box also matches the current custodian's employee code (`Employee.code`) in addition to asset identifiers and custodian name, but intentionally does not search repair reporters, assignees, or other related employees. The Import Wizard starts collapsed and opens on demand, and table column presets (`All`, `Operations`, `Accounting`, `Audit`) persist per browser through local storage.
- Asset Register now accepts master-data drilldown filters from Category and Brand/Model counts. Category model counts open the Brand/Model workspace scoped to that category, category asset counts open `/assets?categoryId=...`, brand asset counts open `/assets?brandId=...`, and model asset counts open `/assets?modelId=...`. Active Brand/Model drilldowns show removable filter chips above the register filters and are preserved through the filter form. The register filter panel uses a balanced five-column desktop grid so the first and second filter rows fill the available width, with status/condition grouped together and compact help icons that do not distort the field rhythm.
