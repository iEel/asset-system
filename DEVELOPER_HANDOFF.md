...
> **Latest Update:** ✅ Asset Batch Create added on `/assets/new`: users can switch to `เพิ่มเป็นชุด`, enter shared purchase/master data once, add 2-100 row-specific Serial Numbers, legacy/custom Asset Tags, Custodians, and Remarks while shared Location and FA/accounting code come from the common batch data, preview/review the shared location, FA/accounting code, auto/manual tag source, Serial, custodian, and remark before final save, auto-generate missing asset tags while reserving manual legacy tags, and continue directly to batch QR label printing.
