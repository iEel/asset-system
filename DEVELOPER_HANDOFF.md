...
| **Lint/build verification** | ล่าสุด `node --test tests\*.test.ts`, `npm run build`, และ HTTP check `/th/dashboard` = 200 ผ่านหลังเพิ่ม Approval Inbox card บน Dashboard; Browser plugin ในเครื่องนี้ไม่มี `scripts/browser-client.mjs` จึงใช้ in-app browser verify ไม่ได้ |
