...
| **Lint/build verification** | ล่าสุด `node --test tests\*.test.ts`, `npm run build`, และ HTTP check `/th/work-center` = 200 ผ่านหลังเพิ่ม Approval Inbox ใน Work Center; Browser plugin ในเครื่องนี้ไม่มี `scripts/browser-client.mjs` จึงใช้ in-app browser verify ไม่ได้ |
