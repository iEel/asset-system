...
> **Latest Update:** ✅ Added permission-aware Global Search. `/api/search` now supports full-system search across assets, employees, suppliers, company/branch/location, maintenance, audit rounds, and disposal requests by permission; asset-only tools pass `scope=asset` to keep scan/label behavior focused.
