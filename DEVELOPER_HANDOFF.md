...
| **Lint/build verification** | ล่าสุด `node --test tests\production-readiness.test.ts tests\approval-permission-matrix.test.ts` และ `npm run build` ผ่านหลังเพิ่ม Production Readiness page; unauthenticated HTTP smoke ของ admin routes redirect/login ตาม session guard จึงใช้ build + helper tests เป็นหลัก |
