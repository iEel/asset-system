...
| **Lint/build verification** | ล่าสุด `node --test tests\*.test.ts`, `npm run build`, และ HTTP check `/th/admin/approvals?module=audit` = 200 ผ่านหลังเพิ่ม filter หน้า Approval Inbox; Browser plugin ในเครื่องนี้ไม่มี `scripts/browser-client.mjs` จึงใช้ in-app browser verify ไม่ได้ |
