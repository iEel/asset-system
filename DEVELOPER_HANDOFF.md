...
> **Latest Update:** ✅ Performance pass for Audit Trail / Approval Decision History label resolution: record/reference id collection is now testable separately and empty module lookups skip Prisma queries.
...
| **Lint/build verification** | ล่าสุด `node --test tests\system-log-record-labels.test.ts tests\system-log-presenter.test.ts`, `node --test tests\*.test.ts` (31 tests pass), และ `npm run build` ผ่านหลัง Performance Pass; unauthenticated HTTP smoke ของ admin routes redirect/login ตาม session guard จึงใช้ build + helper tests เป็นหลัก |
