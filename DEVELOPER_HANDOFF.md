...
> **Latest Update:** ✅ Preventive Maintenance auto-generation pass completed. Added `POST /api/maintenance-plans/generate-due` and `npm run pm:generate-due` for scheduled due-plan generation using `MAINTENANCE_PM_GENERATION_TOKEN`; the job skips open duplicate `[PM]` tickets, skips plans without a reporter, creates tickets for due plans, advances `nextDueDate`, and logs a batch audit summary.
