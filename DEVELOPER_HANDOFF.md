...
> **Latest Update:** ✅ Web-controlled scheduler heartbeat added for PM auto-generation and LDAP Sync: System Settings now owns the cron schedules, `npm run scheduler:heartbeat` wakes both jobs, scheduled API calls skip until due, run status is persisted, and Ubuntu deployment docs now use one shared `systemd timer` instead of separate crontab/PM timers.
