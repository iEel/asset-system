...
- Asset create/edit ownership forms now pass employee department and organization codes into the custodian dropdown. The dropdown filters by selected company, branch, and department, and also tolerates legacy employee rows whose `branchId` points to a stale/duplicate branch record when the employee company and branch code still match the selected scope. This fixes cases such as employee `8171` being hidden after selecting GRANDLINK / SathuPradit / IT.
- Asset lifecycle hardening now blocks checkout for Disposed, Retired, Pending Disposal, Under Maintenance, Lost, and Missing assets, blocks normal transfer for Disposed, Retired, and Pending Disposal assets, interprets PM/LDAP automation cron schedules in `Asia/Bangkok` time, and adds upload validation regressions for oversize, disallowed MIME, and spoofed-extension files.
